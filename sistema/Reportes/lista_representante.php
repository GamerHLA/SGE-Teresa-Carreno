...
class PDF extends FPDF {
    ...
    function Header()
    {
        ...
        global $filtroEstatus, $filtroSexo, $filtroTipo, $filtroEsProfesor;
        ...
        if ($filtroEsProfesor == '1') {
            $titulo .= ' - Son Profesores';
        } elseif ($filtroEsProfesor == '0') {
            $titulo .= ' - No son Profesores';
        }
        ...
        // #, Cédula, Nombre y Apellido, Alumno, Parentesco, Dirección, Teléfono, Correo
        $w = array(12, 20, 34, 32, 20, 30, 22, 26);
        $anchoTotal = array_sum($w);
        $margenIzquierdo = (216 - $anchoTotal) / 2;
        $this->SetX($margenIzquierdo);
        
        $headers = array('#', utf8_decode('Cédula'), 'Nombre y Apellido', 'Alumno', 'Parentesco', utf8_decode('Dirección'), utf8_decode('Teléfono'), 'Correo');
...
$filtroEsProfesor = isset($_GET['es_profesor']) ? $_GET['es_profesor'] : 'all';
...
if ($filtroEstatus !== 'all') {
    $whereClauses[] = "r.estatus = " . intval($filtroEstatus);
} else {
    $whereClauses[] = "r.estatus != 0"; // Excluir eliminados
}

// Filtro de sexo
if ($filtroSexo !== 'all') {
    $whereClauses[] = "r.sexo = '" . $conexion->real_escape_string($filtroSexo) . "'";
}

// Filtro de tipo (principal/secundario) - usando subquery porque es_principal está en alumno_representante
if ($filtroTipo !== 'all') {
    if ($filtroTipo == '1') {
        // Principal: tiene al menos un alumno donde es_principal = 1
        $whereClauses[] = "EXISTS (SELECT 1 FROM alumno_representante ar2 WHERE ar2.representante_id = r.representantes_id AND ar2.es_principal = 1 AND ar2.estatus = 1)";
    } else {
        // Secundario: tiene al menos un alumno donde es_principal = 0
        $whereClauses[] = "EXISTS (SELECT 1 FROM alumno_representante ar2 WHERE ar2.representante_id = r.representantes_id AND ar2.es_principal = 0 AND ar2.estatus = 1)";
    }
}

// Filtro de es_profesor
if ($filtroEsProfesor !== 'all') {
    if ($filtroEsProfesor == '1') {
        $whereClauses[] = "pr.profesor_id IS NOT NULL";
    } else {
        $whereClauses[] = "pr.profesor_id IS NULL";
    }
}
...
$sql = "SELECT 
        ...
        LEFT JOIN profesor pr ON r.cedula = pr.cedula AND pr.estatus = 1
        WHERE $whereSQL
...
$w = array(12, 20, 34, 32, 20, 30, 22, 26);
$pdf->SetWidths($w);
$pdf->SetAligns(array('C','C','L','L','C','L','C','L'));
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        // Formatear teléfono: 0412-0906377
        ...
        // Línea de separación para mejor legibilidad entre representantes
        $pdf->SetX($margenIzquierdo);
...
$conexion->close();
$pdf->Output('lista_representantes.pdf', 'I');
?>