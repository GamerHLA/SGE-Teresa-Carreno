<?php
/**
 * CONSTANCIA_ESTUDIO.PHP
 * ======================
 * 
 * Generador de PDF para constancias de estudio de alumnos.
 * 
 * FUNCIONALIDADES:
 * - Genera constancia oficial de estudio en PDF
 * - Incluye código QR de verificación
 * - Datos del alumno: nombre, cédula, fecha nacimiento, edad, lugar origen
 * - Datos académicos: grado, sección, turno, período escolar
 * - Firma digital del director
 * - Encabezado con logo institucional
 * - Pie de página con datos de contacto
 * 
 * PARÁMETROS:
 * - GET['id']: ID de inscripción del alumno
 * 
 * DEPENDENCIAS:
...
class PDF extends FPDF
{
    ...
    {
        $qrContent = "CONSTANCIA DE ESTUDIO\n";
        $qrContent .= "======================\n";
...
if ($inscripcion_id <= 0) {
    ...
    $pdf->Ln(2);
    $pdf->MultiCellUTF8(0, 8, 'Ejemplo: constancia_estudio.php?id=1', 0, 'C');
    $conexion->close();
    $pdf->cleanupQRFiles();
...
if ($result->num_rows > 0) {
    ...
    $pdf->CellUTF8(0, 10, 'CONSTANCIA DE ESTUDIO', 0, 1, 'C');
    $pdf->Ln(10);
    ...
    $pdf->SetFont('Arial', '', 12);
    $pdf->WriteUTF8(7, ', cursa estudios en este plantel en el ');
    $pdf->SetFont('Arial', 'B', 12);
